...
let Global_Audio_Context = new (window.AudioContext ||
    ...
    if (BackgroundMusicResourcesList[Key]) {
      ...
      GLOBAL_SOUND_CONTROLLER[Key].onplay = () => {
        ...
        if (!GLOBAL_SOUND_CONTROLLER[Key]["_stored_source"]) {
          GLOBAL_SOUND_CONTROLLER[Key]["_stored_source"] =
            Global_Audio_Context.createMediaElementSource(
              GLOBAL_SOUND_CONTROLLER[Key]
            );
        }
        if (!GLOBAL_SOUND_CONTROLLER[Key]["_stored_gainNode"]) {
          GLOBAL_SOUND_CONTROLLER[Key]["_stored_gainNode"] =
            Global_Audio_Context.createGain();
        }
        ...
        CurrentBGMGainNode.gain.setValueAtTime(
          0,
          Global_Audio_Context.currentTime
        );
        CurrentBGMGainNode.gain.linearRampToValueAtTime(
          GLOBAL_SOUND_CONTROLLER[Key]["_stored_maxVolumn"],
          Global_Audio_Context.currentTime + 2
        );
        CurrentBGMGainNode.gain.linearRampToValueAtTime(
          0,
          GLOBAL_SOUND_CONTROLLER[Key].duration - 2
        );
        TSTORE(
          "now_displaying_music",
          ComponentRender(
            LoadedComponentList["_component_music_info"],
            _thisMusic.name,
            _thisMusic.author,
            _thisMusic.album
          )
      ...
      if (reject) {
        reject("Sound Controller:Trying to Load a Non-exist Resource:" + Key);
      } else {
        throw new Error(
          "Sound Controller:Trying to Load a Non-exist Resource:" + Key
        );
      }
    }
  });
