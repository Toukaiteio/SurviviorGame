...
function ComponentRender(Component) {
  ...
  if (Component.is_forced_full_parameter) {
    if (_given_arguments.length !== Component.require_parameters) {
      throw new Error(
        `${Component.name}: Need ${Component.require_parameters} Parameters,Only ${_given_arguments.length} Were Given!`
      );
    }
  }
  for (const i in _given_arguments) {
    if (Component.para_types[i] !== "any")
      if (typeof _given_arguments[i] !== Component.para_types[i]) {
        throw new Error(
          `${Component.name}: Need Type ${Component.para_types[i]} At ${
            i + 1
          } Parameter.The Given Was ${typeof _given_arguments[i]}!`
        );
      }
  }
...
function PageRender(Page) {
  ...
  if (Page.is_forced_full_parameter) {
    if (_given_arguments.length !== Page.require_parameters) {
      throw new Error(
        `${Page.name}: Need ${Page.require_parameters} Parameters,Only ${_given_arguments.length} Were Given!`
      );
    }
  }
  for (const i in _given_arguments) {
    if (Page.para_types[i] !== "any")
      if (typeof _given_arguments[i] !== Page.para_types[i]) {
        throw new Error(
          `${Page.name}: Need Type ${Page.para_types[i]} At ${
            i + 1
          } Parameter.The Given Was ${typeof _given_arguments[i]}!`
        );
      }
  ...
  for (const i of Components) {
    await Load_Script(i).then((Self) => {
      ...
      if (typeof ID === "string") {
        ...
        throw new Error(
          `${COMPONENTNAME}: Didn't Set Its ID Properly!Which It's ID was`,
          ID
        );
      }
      InitialMetaData();
