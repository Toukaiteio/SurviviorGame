...
Render = function () {
  ...
  Home.style.cssText = `
        ...
        width:100vw;
    `;
  PreloadSoundResource("m1").then((Key) => PlayBGM(Key, true));
  const _action_selector = ComponentRender(
    LoadedComponentList["_component_action_selector"]
  );
  //render action selector
  if (!TGET("_home_page_styled")) {
    ApplyStyle(
      "button#_component_action_button.selecting",
      `
  ...
  const main_wrapper = ComponentRender(
    LoadedComponentList["_component_general_wrapper"]
  );
